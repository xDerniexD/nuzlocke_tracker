...
function LegendaryTrackerPage() {
    ...
    useEffect(() => {
        ...
        socket.on('connect', () => socket.emit('joinRoom', runId));
        socket.on('nuzlocke:legendary_updated', (updatedEncounters) => setLegendaryEncounters(updatedEncounters));
    ...
    const handleAddGeneric = (playerId) => handleAddEncounter(0, playerId, null);

    const handleRemoveGeneric = async (playerId) => {
        try {
            const response = await api.delete(`/nuzlockes/${runId}/legendary/generic/${playerId}`);
            setLegendaryEncounters(response.data);
        } catch (err) { if (err.response?.status !== 404) console.error("Fehler beim Entfernen des generischen Counters:", err); }
    };
    ...
    return (
        <Container maxW="container.2xl" py={8}>
            ...
            <Text fontSize="xl" color="gray.500" mb={4}>{t('legendary_tracker.title')}</Text>
            <SubNav />
            <Center mb={8}>
                <ButtonGroup isAttached>
                    <Button onClick={() => setViewMode('simple')} isActive={viewMode === 'simple'}>{t('legendary_tracker.simple')}</Button>
                    <Button onClick={() => setViewMode('expanded')} isActive={viewMode === 'expanded'}>{t('legendary_tracker.expanded')}</Button>
                    <Button onClick={() => setViewMode('detailed')} isActive={viewMode === 'detailed'}>{t('legendary_tracker.detailed')}</Button>
                </ButtonGroup>
            </Center>
            {viewMode === 'simple' && (
                <SimpleGrid columns={{ base: 1, md: run.type === 'soullink' ? 2 : 1 }} spacing={10}>
                    {Object.values(playerCounts).map(playerData => {
                        ...
                        return (
                            <Card key={playerData.username}><CardHeader><Heading size='md'>{playerData.username}</Heading></CardHeader>
                                <CardBody><Flex align="center" justify="space-between">
                                    <Stat><StatNumber fontSize="6xl">{playerData.total}</StatNumber></Stat>
                                    <VStack>
            ...
            {viewMode === 'expanded' && (
                <SimpleGrid columns={{ base: 2, md: 4, lg: 6 }} spacing={4}>
                    {legendaryPokemonList.map(pokemon => (
                        <Card key={pokemon.id} p={2}><VStack>
                                ...
                                {run.participants.map(player => (
                                    <VStack key={player._id} spacing={1}>
                                        ...
                                        <ButtonGroup isAttached size="sm">
                                            ...
                                            <Button variant="outline" cursor="default">{playerCounts[player._id]?.byPokemon[pokemon.id] || 0}</Button>
            ...
            {viewMode === 'detailed' && (
                <SimpleGrid columns={{ base: 2, md: 4, lg: 6 }} spacing={4}>
                    {legendaryPokemonList.map(pokemon => (
                        <Card key={pokemon.id} p={2} cursor="pointer" onClick={() => openDetailedModal(pokemon)} _hover={{ bg: "gray.100", _dark: { bg: "gray.700" } }}><VStack>
                            <PokemonSprite pokemonId={pokemon.id} /><Text fontWeight="bold">{pokemon.name}</Text>
                            <HStack>
            ...
            <Modal isOpen={isOpen} onClose={onClose}>
                <ModalOverlay />
                <ModalContent>
                    <ModalHeader>{t('legendary_tracker.add_encounter_header', { pokemonName: selectedPokemon?.name })}</ModalHeader><ModalCloseButton />
                    <ModalBody><VStack spacing={4}>
                        {run.type === 'soullink' && (
                            <FormControl as="fieldset">
                                <FormLabel as="legend">{t('legendary_tracker.player_label')}</FormLabel>
                                <RadioGroup onChange={setSelectedPlayerId} value={selectedPlayerId}>
                                    <HStack spacing={4}>{run.participants.map(p => <Radio key={p._id} value={p._id}>{p.username}</Radio>)}</HStack>
                                </RadioGroup>
                            </FormControl>
                        )}
                        <FormControl>
                            <FormLabel>{t('legendary_tracker.encounter_type_label')}</FormLabel>
                            <Input value={encounterType} onChange={(e) => setEncounterType(e.target.value)} placeholder={t('legendary_tracker.encounter_type_placeholder')} />
                        </FormControl>
                    </VStack></ModalBody>
                    <ModalFooter>
                        <Button variant="ghost" mr={3} onClick={onClose}>{t('rules.close_button')}</Button>
                        <Button colorScheme="teal" onClick={confirmDetailedEncounter}>{t('rules.save_button')}</Button>
                    </ModalFooter>
                </ModalContent>
            </Modal>
        </Container>
