...
// Dieselbe Liste wie im LegendaryTracker, um Konsistenz zu gewährleisten
...
function SpectatorPage() {
  ...
  // States für den Team-Builder
  const [team, setTeam] = useState([]);
  const [box, setBox] = useState([]);
  const [fainted, setFainted] = useState([]);
  const [missed, setMissed] = useState([]);

  // NEU: State und Sub-Ansicht für den Legendary Tracker
  ...
  useEffect(() => {
    ...
    // NEU: WebSocket-Listener für legendäre Pokémon
    socket.on('nuzlocke:legendary_updated', (updatedLegendaryEncounters) => {
      setLegendaryEncounters(updatedLegendaryEncounters);
    });
  ...
  // NEU: Berechnung der Zähler für den Legendary Tracker
  const playerCounts = useMemo(() => {
    const counts = {};
    if (run) {
      run.participants.forEach(p => {
        counts[p._id] = { username: p.username, total: 0, byPokemon: {} };
      });
    ...
    <Container maxW="container.2xl" py={8}>
      ...
      <Flex justifyContent="center" my={6}>
        <ButtonGroup isAttached variant="outline">
          ...
          <Button onClick={() => setView('legendaries')} isActive={view === 'legendaries'}>Legendaries</Button>
        </ButtonGroup>
      </Flex>
      ...
      {/* NEU: Ansicht für den Legendary Tracker */}
      {view === 'legendaries' && (
        <Box>
          <Center mb={8}>
            <ButtonGroup isAttached variant="outline">
              <Button onClick={() => setLegendaryViewMode('simple')} isActive={legendaryViewMode === 'simple'}>Einfach</Button>
              <Button onClick={() => setLegendaryViewMode('expanded')} isActive={legendaryViewMode === 'expanded'}>Erweitert</Button>
              <Button onClick={() => setLegendaryViewMode('detailed')} isActive={legendaryViewMode === 'detailed'}>Detailliert</Button>
            </ButtonGroup>
          </Center>
            ...
            <SimpleGrid columns={{ base: 1, md: run.type === 'soullink' ? 2 : 1 }} spacing={10}>
              {Object.values(playerCounts).map(playerData => (
                <Card key={playerData.username}>
                  <CardHeader><Heading size='md'>{playerData.username}</Heading></CardHeader>
                  <CardBody><Stat><StatNumber fontSize="6xl">{playerData.total}</StatNumber></Stat></CardBody>
                </Card>
              ))}
            </SimpleGrid>
          )}
                    ...
                    <HStack>
                      {run.participants.map(player => (
                        <VStack key={player._id} spacing={1}>
                          ...
                          <Button variant="outline" cursor="default" size="sm">
