...
function PokemonDetailModal({ isOpen, onClose, pokemon, isLoading, game }) {
  ...
  useEffect(() => {
    if (pokemon && pokemon.evolutionChainId) {
      const fetchEvolutionChain = async () => {
        ...
        } catch (error) { setEvolutionChain(null); }
  ...
  const handleAbilityClick = (ability) => { setSelectedAbility(ability); onAbilityOpen(); };
  const getEvolutionTriggerText = (details) => {
    if (!details) return '';
    const trigger = details.trigger.name;
    switch (trigger) {
      case 'level-up': return t('evolution_triggers.level-up', { level: details.min_level });
      case 'trade': return t('evolution_triggers.trade');
      case 'use-item':
        const itemName = (i18n.language === 'de' && details.item.name_de) ? details.item.name_de : details.item.name.replace(/-/g, ' ');
        return t('evolution_triggers.use-item', { item: itemName });
      default: return t('evolution_triggers.other');
    }
  };
  ...
  const groupedGameMoves = useMemo(() => {
    ...
    Object.keys(movesByMethod).forEach(method => {
      if (movesByMethod[method].length > 0) {
        grouped[method] = [...movesByMethod[method]].sort((a, b) => {
          if (method === 'level-up') return a.level - b.level;
          const nameA = a.move?.name_en || '';
          const nameB = b.move?.name_en || '';
  ...
  return (
    <>
      <Modal isOpen={isOpen} onClose={onClose} size="4xl" isCentered motionPreset="slideInBottom">
        <ModalOverlay />
        <ModalContent>
          {isLoading ? (<Flex justify="center" align="center" height="400px"><Spinner size="xl" /></Flex>) :
            pokemon ? (
              <>
                ...
                <ModalBody>
                  <Grid templateColumns={{ base: "1fr", md: "1fr 2fr" }} gap={8}>
                    <VStack spacing={5} align="stretch">
                      <Center><Image src={pokemon.spriteUrl || `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${pokemon.id}.png`} boxSize="150px" imageRendering="pixelated" crossOrigin="anonymous" /></Center>
                          ...
                          <HStack justify="center" align="center" spacing={1}>
                            {evolutionChain && evolutionChain.length > 1 ? evolutionChain.map((evo, index) => (
                              <React.Fragment key={evo.pokemonId}>
                                {index > 0 && (<VStack px={2} minW="80px"><Icon as={FaArrowRight} /><Tag size="sm" mt={1} whiteSpace="nowrap" textTransform="capitalize">{getEvolutionTriggerText(evo.evolution_details)}</Tag></VStack>)}
                                <VStack><PokemonSprite pokemonId={evo.pokemonId} /><Text fontSize="sm">{(i18n.language === 'de' && evo.name_de) ? evo.name_de : evo.name_en}</Text></VStack>
                              </React.Fragment>
                      ...
                      <Box>
                        ...
                        <VStack spacing={3} align="stretch">{pokemon.baseStats && Object.entries(pokemon.baseStats).map(([stat, value]) => (<Grid key={stat} templateColumns="110px 1fr 40px" gap={3} alignItems="center"><Text fontSize="sm" textAlign="right" fontWeight="medium">{formatStatName(stat)}</Text><Progress value={value} max={255} colorScheme={value < 60 ? 'red' : value < 90 ? 'yellow' : 'green'} size="sm" borderRadius="md" /><Text fontSize="sm" fontWeight="bold">{value}</Text></Grid>))}</VStack>
                      </Box>
                      <Box>
                                  ...
                                  <Box maxH="450px" overflowY="auto" pr={2}>
                                    <VStack spacing={0} align="stretch">
                                      <Grid templateColumns={moveGridColumns} gap={3} fontWeight="bold" fontSize="xs" color="gray.500" position="sticky" top={0} bg="white" _dark={{ bg: 'gray.800' }} py={2} zIndex={1} textAlign="center">
                                        <Text textAlign="left">{method === 'machine' ? t('modal.tm_hm') : t('modal.level')}</Text>
                                        <Text textAlign="left">{t('modal.move')}</Text>
                                        <Text>{t('modal.type')}</Text><Text>{t('modal.category')}</Text><Text>{t('modal.power')}</Text><Text>{t('modal.accuracy')}</Text><Text>{t('modal.pp')}</Text>
                                      </Grid>
                                      {groupedGameMoves[method].map((moveSlot, index) => {
                                        ...
                                        return (
                                          <Grid key={`${move._id}-${index}`} templateColumns={moveGridColumns} gap={3} fontSize="sm" py={2} borderTopWidth={1} borderColor="gray.100" _dark={{ borderColor: "gray.600" }} alignItems="center" textAlign="center" onClick={() => handleMoveClick(move)} cursor="pointer" _hover={{ bg: 'gray.100', _dark: { bg: 'gray.700' } }} borderRadius="md">
                                            <Text fontWeight="bold" textAlign="left">{method === 'machine' ? (moveSlot.tm_name || '—') : (moveSlot.level > 0 ? moveSlot.level : '—')}</Text>
